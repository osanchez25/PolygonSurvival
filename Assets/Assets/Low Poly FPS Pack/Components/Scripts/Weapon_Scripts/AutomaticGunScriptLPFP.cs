...
public class AutomaticGunScriptLPFP : MonoBehaviour {
	...
	public int StoredAmmo = 120;
	public Text storedAmmoText;
	...
	public int granadeAmmo = 3;

	public Text storedGranadeAmmoText;
	...
	private void Start () {
		...
		storedGranadeAmmoText.text = granadeAmmo.ToString();
	...
	public void KnifeAttack()
	{
		...
		Collider[] colliders = Physics.OverlapSphere(explosionPos, 3f);
		...
		{	
			...
			if (hit.GetComponent<Collider>().tag == "Blood")
			{
				...
				if (hit.gameObject.layer == LayerMask.NameToLayer("Head"))
				{
					EnemyHealth health = hit.gameObject.GetComponentInParent<EnemyHealth>();

					EnemyMovement enemyMovement = hit.gameObject.GetComponentInParent<EnemyMovement>();
					if (enemyMovement != null)
					{
						enemyMovement.KnockBack(5f);
					}
